// <auto-generated>
//      SuperOffice Code Generator
//      modelling\templates\servicesnew\tswebapi.tt
// </auto-generated>


/* tslint:disable */
//----------------------------------------------------------------------------
// G E N E R A T E D   F I L E !                       D O   N O T   E D I T !
//----------------------------------------------------------------------------
//
// Generated:
// File:          WebApi/WebApi.cs
//
//----------------------------------------------------------------------------
//                                                Copyright (C) SuperOffice AS
//----------------------------------------------------------------------------

import * as base from './WebApiBase';
import { AxiosRequestConfig } from 'axios';
import { IQuoteAgent, QuoteAgent } from './QuoteAgent';
export { QuoteAgent };
import { IErpSyncAgent, ErpSyncAgent } from './ErpSyncAgent';
export { ErpSyncAgent };
import { IResourceAgent, ResourceAgent } from './ResourceAgent';
export { ResourceAgent };
import { IContactAgent, ContactAgent } from './ContactAgent';
export { ContactAgent };
import { IPersonAgent, PersonAgent } from './PersonAgent';
export { PersonAgent };
import { ISaleAgent, SaleAgent } from './SaleAgent';
export { SaleAgent };
import { IPhoneListAgent, PhoneListAgent } from './PhoneListAgent';
export { PhoneListAgent };
import { IProjectAgent, ProjectAgent } from './ProjectAgent';
export { ProjectAgent };
import { IAppointmentAgent, AppointmentAgent } from './AppointmentAgent';
export { AppointmentAgent };
import { IForeignSystemAgent, ForeignSystemAgent } from './ForeignSystemAgent';
export { ForeignSystemAgent };
import { IDocumentAgent, DocumentAgent } from './DocumentAgent';
export { DocumentAgent };
import { IListAgent, ListAgent } from './ListAgent';
export { ListAgent };
import { IBLOBAgent, BLOBAgent } from './BLOBAgent';
export { BLOBAgent };
import { INavigatorAgent, NavigatorAgent } from './NavigatorAgent';
export { NavigatorAgent };
import { IMDOAgent, MDOAgent } from './MDOAgent';
export { MDOAgent };
import { IConfigurationAgent, ConfigurationAgent } from './ConfigurationAgent';
export { ConfigurationAgent };
import { IViewStateAgent, ViewStateAgent } from './ViewStateAgent';
export { ViewStateAgent };
import { IAssociateAgent, AssociateAgent } from './AssociateAgent';
export { AssociateAgent };
import { IDiagnosticsAgent, DiagnosticsAgent } from './DiagnosticsAgent';
export { DiagnosticsAgent };
import { ITooltipsAgent, TooltipsAgent } from './TooltipsAgent';
export { TooltipsAgent };
import { IPreviewsAgent, PreviewsAgent } from './PreviewsAgent';
export { PreviewsAgent };
import { ISelectionAgent, SelectionAgent } from './SelectionAgent';
export { SelectionAgent };
import { IEMailAgent, EMailAgent } from './EMailAgent';
export { EMailAgent };
import { IFindAgent, FindAgent } from './FindAgent';
export { FindAgent };
import { IWebhookAgent, WebhookAgent } from './WebhookAgent';
export { WebhookAgent };
import { ISentryAgent, SentryAgent } from './SentryAgent';
export { SentryAgent };
import { IUserDefinedFieldInfoAgent, UserDefinedFieldInfoAgent } from './UserDefinedFieldInfoAgent';
export { UserDefinedFieldInfoAgent };
import { IReportAgent, ReportAgent } from './ReportAgent';
export { ReportAgent };
import { ISaintAgent, SaintAgent } from './SaintAgent';
export { SaintAgent };
import { IBatchAgent, BatchAgent } from './BatchAgent';
export { BatchAgent };
import { IRelationAgent, RelationAgent } from './RelationAgent';
export { RelationAgent };
import { IArchiveAgent, ArchiveAgent } from './ArchiveAgent';
export { ArchiveAgent };
import { IAudienceAgent, AudienceAgent } from './AudienceAgent';
export { AudienceAgent };
import { IPreferenceAgent, PreferenceAgent } from './PreferenceAgent';
export { PreferenceAgent };
import { ILicenseAgent, LicenseAgent } from './LicenseAgent';
export { LicenseAgent };
import { IUserAgent, UserAgent } from './UserAgent';
export { UserAgent };
import { IReplicationAgent, ReplicationAgent } from './ReplicationAgent';
export { ReplicationAgent };
import { ITimeZoneAgent, TimeZoneAgent } from './TimeZoneAgent';
export { TimeZoneAgent };
import { IMessagingAgent, MessagingAgent } from './MessagingAgent';
export { MessagingAgent };
import { IImportAgent, ImportAgent } from './ImportAgent';
export { ImportAgent };
import { IFreeTextAgent, FreeTextAgent } from './FreeTextAgent';
export { FreeTextAgent };
import { INumberAllocationAgent, NumberAllocationAgent } from './NumberAllocationAgent';
export { NumberAllocationAgent };
import { ICustomerServiceAgent, CustomerServiceAgent } from './CustomerServiceAgent';
export { CustomerServiceAgent };
import { IPocketAgent, PocketAgent } from './PocketAgent';
export { PocketAgent };
import { IDashboardAgent, DashboardAgent } from './DashboardAgent';
export { DashboardAgent };
import { IDashAgent, DashAgent } from './DashAgent';
export { DashAgent };
import { ITargetsAgent, TargetsAgent } from './TargetsAgent';
export { TargetsAgent };
import { IFavouriteAgent, FavouriteAgent } from './FavouriteAgent';
export { FavouriteAgent };
import { IDatabaseAgent, DatabaseAgent } from './DatabaseAgent';
export { DatabaseAgent };
import { IBulkUpdateAgent, BulkUpdateAgent } from './BulkUpdateAgent';
export { BulkUpdateAgent };
import { IMarketingAgent, MarketingAgent } from './MarketingAgent';
export { MarketingAgent };
import { ICRMScriptAgent, CRMScriptAgent } from './CRMScriptAgent';
export { CRMScriptAgent };
import { ITicketAgent, TicketAgent } from './TicketAgent';
export { TicketAgent };
import { IDatabaseTableAgent, DatabaseTableAgent } from './DatabaseTableAgent';
export { DatabaseTableAgent };
import { IChatAgent, ChatAgent } from './ChatAgent';
export { ChatAgent };
import { IAIAgent, AIAgent } from './AIAgent';
export { AIAgent };
import { IDocumentMigrationAgent, DocumentMigrationAgent } from './DocumentMigrationAgent';
export { DocumentMigrationAgent };
import { IWorkflowAgent, WorkflowAgent } from './WorkflowAgent';
export { WorkflowAgent };

import { IContactRestApi, ContactRestApi } from './ContactRestApi';
export { ContactRestApi };
import { IPersonRestApi, PersonRestApi } from './PersonRestApi';
export { PersonRestApi };
import { ISaleRestApi, SaleRestApi } from './SaleRestApi';
export { SaleRestApi };
import { IProjectRestApi, ProjectRestApi } from './ProjectRestApi';
export { ProjectRestApi };
import { IAppointmentRestApi, AppointmentRestApi } from './AppointmentRestApi';
export { AppointmentRestApi };
import { IDocumentRestApi, DocumentRestApi } from './DocumentRestApi';
export { DocumentRestApi };

// Re-export useful things from base 
import { WebApiStatus, WebApiRequestOptions, WebApiOptions, ResourceParsing } from './WebApiBase';
export { WebApiStatus, WebApiRequestOptions, WebApiOptions, ResourceParsing };


/**
 * WebApi Agents connection - configure base URL and authentication here, then get the agents you want to use.
 */
export interface IAgentsApi extends base.WebApiBase {
  /** Services for the Quote Management feature, part of the Sale module */
  getQuoteAgent(resourceParsing?: base.ResourceParsing) : IQuoteAgent;
  /** Services for the ERP Integration Services feature */
  getErpSyncAgent(resourceParsing?: base.ResourceParsing) : IErpSyncAgent;
  /** String resource substitution management. */
  getResourceAgent(resourceParsing?: base.ResourceParsing) : IResourceAgent;
  /** Contact (company) data services. */
  getContactAgent(resourceParsing?: base.ResourceParsing) : IContactAgent;
  /** Person data services. */
  getPersonAgent(resourceParsing?: base.ResourceParsing) : IPersonAgent;
  /** Sale data services */
  getSaleAgent(resourceParsing?: base.ResourceParsing) : ISaleAgent;
  /** Collection of all services for searching for person or contact (company) phone numbers. */
  getPhoneListAgent(resourceParsing?: base.ResourceParsing) : IPhoneListAgent;
  /** Collection of all services that works with Project data. */
  getProjectAgent(resourceParsing?: base.ResourceParsing) : IProjectAgent;
  /** Collection of all services that works with Appointment data. */
  getAppointmentAgent(resourceParsing?: base.ResourceParsing) : IAppointmentAgent;
  /** Collection of all services that works with Foreign key data (Key/Value pairs) */
  getForeignSystemAgent(resourceParsing?: base.ResourceParsing) : IForeignSystemAgent;
  /** Collection of all services that works with Document data. This is services for the document information, not the physical document themselves. These are handled by the BLOB service methods. */
  getDocumentAgent(resourceParsing?: base.ResourceParsing) : IDocumentAgent;
  /** Collection of all services that works with Lists. These are typical lists of data shown in dropdown list, checkbox lists, etc. */
  getListAgent(resourceParsing?: base.ResourceParsing) : IListAgent;
  /** Collection of all services that works with binary objects (BLOBS), e.g. Images and documents. */
  getBLOBAgent(resourceParsing?: base.ResourceParsing) : IBLOBAgent;
  /** Navigator stuff. */
  getNavigatorAgent(resourceParsing?: base.ResourceParsing) : INavigatorAgent;
  /** MDO Lists, reading, searching, and item lookup. */
  getMDOAgent(resourceParsing?: base.ResourceParsing) : IMDOAgent;
  /** User interface configuration - XMLs and other elements such as inter-client URLs */
  getConfigurationAgent(resourceParsing?: base.ResourceParsing) : IConfigurationAgent;
  /** User interface view state, history, currentXXX values */
  getViewStateAgent(resourceParsing?: base.ResourceParsing) : IViewStateAgent;
  /** Associate utilities, notes, not user admininstration */
  getAssociateAgent(resourceParsing?: base.ResourceParsing) : IAssociateAgent;
  /** Diagnostics, usage data collection, caches and flushing */
  getDiagnosticsAgent(resourceParsing?: base.ResourceParsing) : IDiagnosticsAgent;
  /** Tooltip system, hint to text transformation */
  getTooltipsAgent(resourceParsing?: base.ResourceParsing) : ITooltipsAgent;
  /** Get preview strings from a hint */
  getPreviewsAgent(resourceParsing?: base.ResourceParsing) : IPreviewsAgent;
  /** Selections, entities, members and tasks */
  getSelectionAgent(resourceParsing?: base.ResourceParsing) : ISelectionAgent;
  /** Email connection, reading, sending */
  getEMailAgent(resourceParsing?: base.ResourceParsing) : IEMailAgent;
  /** Find functions */
  getFindAgent(resourceParsing?: base.ResourceParsing) : IFindAgent;
  /** Webhook definitions - webhooks signal other systems about events inside NetServer */
  getWebhookAgent(resourceParsing?: base.ResourceParsing) : IWebhookAgent;
  /** Data and function right queries */
  getSentryAgent(resourceParsing?: base.ResourceParsing) : ISentryAgent;
  /** Returns information about user-defined fields, like field type, field size, field label text, default value. Does not return the actual values in the user-defined fields. The values are returned on the entity objects. */
  getUserDefinedFieldInfoAgent(resourceParsing?: base.ResourceParsing) : IUserDefinedFieldInfoAgent;
  /** Run reports, set favourites, labels */
  getReportAgent(resourceParsing?: base.ResourceParsing) : IReportAgent;
  /** Administration and maintenance of SAINT counters and statuses */
  getSaintAgent(resourceParsing?: base.ResourceParsing) : ISaintAgent;
  /** Start, stop and monitor predefined batch tasks */
  getBatchAgent(resourceParsing?: base.ResourceParsing) : IBatchAgent;
  /** Contact/Person relations */
  getRelationAgent(resourceParsing?: base.ResourceParsing) : IRelationAgent;
  /** ArchiveList functions of all kinds */
  getArchiveAgent(resourceParsing?: base.ResourceParsing) : IArchiveAgent;
  /** Services specific to the Audience client */
  getAudienceAgent(resourceParsing?: base.ResourceParsing) : IAudienceAgent;
  /** Preferences, user interface tab configuration */
  getPreferenceAgent(resourceParsing?: base.ResourceParsing) : IPreferenceAgent;
  /** License query, download, activation */
  getLicenseAgent(resourceParsing?: base.ResourceParsing) : ILicenseAgent;
  /** User administration */
  getUserAgent(resourceParsing?: base.ResourceParsing) : IUserAgent;
  /** Replication/Travel administration */
  getReplicationAgent(resourceParsing?: base.ResourceParsing) : IReplicationAgent;
  /** TimeZone maintenance */
  getTimeZoneAgent(resourceParsing?: base.ResourceParsing) : ITimeZoneAgent;
  /** SMS and other external messaging systems */
  getMessagingAgent(resourceParsing?: base.ResourceParsing) : IMessagingAgent;
  /** This agent can be used to import data into the system */
  getImportAgent(resourceParsing?: base.ResourceParsing) : IImportAgent;
  /** This agent can be used to manage the free text system */
  getFreeTextAgent(resourceParsing?: base.ResourceParsing) : IFreeTextAgent;
  /** This agent can be used to manage number allocation */
  getNumberAllocationAgent(resourceParsing?: base.ResourceParsing) : INumberAllocationAgent;
  /** Agent used for Customer Service methods */
  getCustomerServiceAgent(resourceParsing?: base.ResourceParsing) : ICustomerServiceAgent;
  /** Agent with Pocket specific functionality */
  getPocketAgent(resourceParsing?: base.ResourceParsing) : IPocketAgent;
  /** Agent lets you configure dashboard tiles and retrieve dashboard data */
  getDashboardAgent(resourceParsing?: base.ResourceParsing) : IDashboardAgent;
  /** Agent that lets you configure dashboard tiles and retrieve dashboard data */
  getDashAgent(resourceParsing?: base.ResourceParsing) : IDashAgent;
  /** Agent lets you configure targets, and retrieve targets */
  getTargetsAgent(resourceParsing?: base.ResourceParsing) : ITargetsAgent;
  /** Agent used for retrieveing and setting favourites */
  getFavouriteAgent(resourceParsing?: base.ResourceParsing) : IFavouriteAgent;
  /** Database schema queries and changes */
  getDatabaseAgent(resourceParsing?: base.ResourceParsing) : IDatabaseAgent;
  /** Agent used for bulk update methods */
  getBulkUpdateAgent(resourceParsing?: base.ResourceParsing) : IBulkUpdateAgent;
  /** Agent used for Marketing functions, such as Forms */
  getMarketingAgent(resourceParsing?: base.ResourceParsing) : IMarketingAgent;
  /** Manage and execute CRMScript functions. */
  getCRMScriptAgent(resourceParsing?: base.ResourceParsing) : ICRMScriptAgent;
  /** Agent used for Ticket functions */
  getTicketAgent(resourceParsing?: base.ResourceParsing) : ITicketAgent;
  /** Agent used for CRUD operations on database tables. Read, add, update, delete rows in tables. */
  getDatabaseTableAgent(resourceParsing?: base.ResourceParsing) : IDatabaseTableAgent;
  /** Chat functions. Manage chat channels, sessions and messages. */
  getChatAgent(resourceParsing?: base.ResourceParsing) : IChatAgent;
  /** AI services, such as Translation, Statistics, Sentiment analysis, backed by calls to Public Cloud providers */
  getAIAgent(resourceParsing?: base.ResourceParsing) : IAIAgent;
  /** Agent used to support migrating documents between different document-plugins, as single documents or batches. */
  getDocumentMigrationAgent(resourceParsing?: base.ResourceParsing) : IDocumentMigrationAgent;
  /** Agent lets you configure workflow automation */
  getWorkflowAgent(resourceParsing?: base.ResourceParsing) : IWorkflowAgent;


  /** The Contact Service. The service implements all services working with the Contact object */
  getContactRestApi(resourceParsing?: base.ResourceParsing) : IContactRestApi;


  /** The Person Service. The service implements all services working with the Person object. */
  getPersonRestApi(resourceParsing?: base.ResourceParsing) : IPersonRestApi;


  /** The Sale Entity contains the sale amount, currency, and sale members. Sales are linked to contacts, persons, and/or projects. */
  getSaleRestApi(resourceParsing?: base.ResourceParsing) : ISaleRestApi;


  /** The Project Service. The service implements all services working with the Project object */
  getProjectRestApi(resourceParsing?: base.ResourceParsing) : IProjectRestApi;


  /** Appointments appear in the diary, and have links to a Contact/Person and possibly a Project or Sale. They have start and end time+date. */
  getAppointmentRestApi(resourceParsing?: base.ResourceParsing) : IAppointmentRestApi;


  /**  */
  getDocumentRestApi(resourceParsing?: base.ResourceParsing) : IDocumentRestApi;



} // interface IWebApi




/**
 * WebApi Agents connection - configure base URL and authentication here, then get the agents you want to use.
 */
export class AgentsApi extends base.WebApiBase implements IAgentsApi {
    constructor(baseUrl?: string, languageCode?:string, cultureCode?: string);
    constructor(baseUrl?: string, resourceParsing?: base.ResourceParsing);
    constructor(baseUrl: string, resourceManager:any);
    constructor(baseUrl: string, config:AxiosRequestConfig, resourceManager:any);
    constructor(baseUrl: string, config:AxiosRequestConfig, resourceParsing?: base.ResourceParsing);
    constructor(baseUrl: string, config:AxiosRequestConfig, languageCode?:string, cultureCode?: string);
    constructor(options: base.WebApiOptions, config?:AxiosRequestConfig);

    constructor(baseUrlOrOptions: string|base.WebApiOptions|undefined,
                langOrconfigOrRM?: string|base.ResourceParsing|AxiosRequestConfig|any, 
                langOrResourceOrRM?:string|base.ResourceParsing|any, 
                cultureCode?: string, 
                ) {
    base.WebApiBase._global_options = undefined;
    let options = base.WebApiBase.FigureOutOptions(baseUrlOrOptions, langOrconfigOrRM, langOrResourceOrRM, cultureCode);
    super(options, false);
  }

  /** Define global options for webapi agents. Once called, you don't need to pass baseUrl to agents. 
   *  This configuration also happens first time an agent is constructed. 
   */
  public static configure(baseUrlOrOptions: string|base.WebApiOptions, configOrOptions?:AxiosRequestConfig|base.WebApiOptions, resourceManagerOrOptions?:any ) {
    base.WebApiBase._global_options = undefined;
    const options = base.WebApiBase.FigureOutOptions(baseUrlOrOptions, configOrOptions, resourceManagerOrOptions); 
    base.WebApiBase._global_options = options;
  }

  /** Authenticate with Basic username:password value. Will not work in CRM Online. */
  public authenticateWithPassword(username:string, password:string) : void {
    let opts = base.WebApiBase._global_options;
    opts = opts || this._options || {};
    opts.axiosConfig = opts.axiosConfig || {};
    opts.axiosConfig.auth = { username: username, password: password };
  }

  /** Authenticate with SOTicket value. */
  public authenticateWithTicket(ticket:string) : void {
    let opts = base.WebApiBase._global_options;
    opts = opts || this._options || {};
    opts.axiosConfig = opts.axiosConfig || {};
    opts.axiosConfig.headers = opts.axiosConfig.headers || {};
    opts.axiosConfig.headers["Authorization"] = "SOTicket " + ticket;
  }

  /** Authenticate with Bearer token */
  public authenticateWithToken(token:string) : void {
    let opts = base.WebApiBase._global_options;
    opts = opts || this._options || {};
    opts.axiosConfig = opts.axiosConfig || {};
    opts.axiosConfig.headers = opts.axiosConfig.headers || {};
    opts.axiosConfig.headers["Authorization"] = "Bearer " + token;
  }

  /** Set SO-AppToken header when talking to CRM Online */
  public applicationToken(token:string) : void {
    let opts = base.WebApiBase._global_options;
    opts = opts || this._options || {};
    opts.axiosConfig = opts.axiosConfig || {};
    opts.axiosConfig.headers = opts.axiosConfig.headers || {};
    opts.axiosConfig.headers["SO-AppToken"] = token;
  }

  /** Services for the Quote Management feature, part of the Sale module */
  public getQuoteAgent(resourceParsing?: base.ResourceParsing) : IQuoteAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new QuoteAgent( options );
  }

  /** Services for the ERP Integration Services feature */
  public getErpSyncAgent(resourceParsing?: base.ResourceParsing) : IErpSyncAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new ErpSyncAgent( options );
  }

  /** String resource substitution management. */
  public getResourceAgent(resourceParsing?: base.ResourceParsing) : IResourceAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new ResourceAgent( options );
  }

  /** Contact (company) data services. */
  public getContactAgent(resourceParsing?: base.ResourceParsing) : IContactAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new ContactAgent( options );
  }

  /** Person data services. */
  public getPersonAgent(resourceParsing?: base.ResourceParsing) : IPersonAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new PersonAgent( options );
  }

  /** Sale data services */
  public getSaleAgent(resourceParsing?: base.ResourceParsing) : ISaleAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new SaleAgent( options );
  }

  /** Collection of all services for searching for person or contact (company) phone numbers. */
  public getPhoneListAgent(resourceParsing?: base.ResourceParsing) : IPhoneListAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new PhoneListAgent( options );
  }

  /** Collection of all services that works with Project data. */
  public getProjectAgent(resourceParsing?: base.ResourceParsing) : IProjectAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new ProjectAgent( options );
  }

  /** Collection of all services that works with Appointment data. */
  public getAppointmentAgent(resourceParsing?: base.ResourceParsing) : IAppointmentAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new AppointmentAgent( options );
  }

  /** Collection of all services that works with Foreign key data (Key/Value pairs) */
  public getForeignSystemAgent(resourceParsing?: base.ResourceParsing) : IForeignSystemAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new ForeignSystemAgent( options );
  }

  /** Collection of all services that works with Document data. This is services for the document information, not the physical document themselves. These are handled by the BLOB service methods. */
  public getDocumentAgent(resourceParsing?: base.ResourceParsing) : IDocumentAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new DocumentAgent( options );
  }

  /** Collection of all services that works with Lists. These are typical lists of data shown in dropdown list, checkbox lists, etc. */
  public getListAgent(resourceParsing?: base.ResourceParsing) : IListAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new ListAgent( options );
  }

  /** Collection of all services that works with binary objects (BLOBS), e.g. Images and documents. */
  public getBLOBAgent(resourceParsing?: base.ResourceParsing) : IBLOBAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new BLOBAgent( options );
  }

  /** Navigator stuff. */
  public getNavigatorAgent(resourceParsing?: base.ResourceParsing) : INavigatorAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new NavigatorAgent( options );
  }

  /** MDO Lists, reading, searching, and item lookup. */
  public getMDOAgent(resourceParsing?: base.ResourceParsing) : IMDOAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new MDOAgent( options );
  }

  /** User interface configuration - XMLs and other elements such as inter-client URLs */
  public getConfigurationAgent(resourceParsing?: base.ResourceParsing) : IConfigurationAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new ConfigurationAgent( options );
  }

  /** User interface view state, history, currentXXX values */
  public getViewStateAgent(resourceParsing?: base.ResourceParsing) : IViewStateAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new ViewStateAgent( options );
  }

  /** Associate utilities, notes, not user admininstration */
  public getAssociateAgent(resourceParsing?: base.ResourceParsing) : IAssociateAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new AssociateAgent( options );
  }

  /** Diagnostics, usage data collection, caches and flushing */
  public getDiagnosticsAgent(resourceParsing?: base.ResourceParsing) : IDiagnosticsAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new DiagnosticsAgent( options );
  }

  /** Tooltip system, hint to text transformation */
  public getTooltipsAgent(resourceParsing?: base.ResourceParsing) : ITooltipsAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new TooltipsAgent( options );
  }

  /** Get preview strings from a hint */
  public getPreviewsAgent(resourceParsing?: base.ResourceParsing) : IPreviewsAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new PreviewsAgent( options );
  }

  /** Selections, entities, members and tasks */
  public getSelectionAgent(resourceParsing?: base.ResourceParsing) : ISelectionAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new SelectionAgent( options );
  }

  /** Email connection, reading, sending */
  public getEMailAgent(resourceParsing?: base.ResourceParsing) : IEMailAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new EMailAgent( options );
  }

  /** Find functions */
  public getFindAgent(resourceParsing?: base.ResourceParsing) : IFindAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new FindAgent( options );
  }

  /** Webhook definitions - webhooks signal other systems about events inside NetServer */
  public getWebhookAgent(resourceParsing?: base.ResourceParsing) : IWebhookAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new WebhookAgent( options );
  }

  /** Data and function right queries */
  public getSentryAgent(resourceParsing?: base.ResourceParsing) : ISentryAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new SentryAgent( options );
  }

  /** Returns information about user-defined fields, like field type, field size, field label text, default value. Does not return the actual values in the user-defined fields. The values are returned on the entity objects. */
  public getUserDefinedFieldInfoAgent(resourceParsing?: base.ResourceParsing) : IUserDefinedFieldInfoAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new UserDefinedFieldInfoAgent( options );
  }

  /** Run reports, set favourites, labels */
  public getReportAgent(resourceParsing?: base.ResourceParsing) : IReportAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new ReportAgent( options );
  }

  /** Administration and maintenance of SAINT counters and statuses */
  public getSaintAgent(resourceParsing?: base.ResourceParsing) : ISaintAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new SaintAgent( options );
  }

  /** Start, stop and monitor predefined batch tasks */
  public getBatchAgent(resourceParsing?: base.ResourceParsing) : IBatchAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new BatchAgent( options );
  }

  /** Contact/Person relations */
  public getRelationAgent(resourceParsing?: base.ResourceParsing) : IRelationAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new RelationAgent( options );
  }

  /** ArchiveList functions of all kinds */
  public getArchiveAgent(resourceParsing?: base.ResourceParsing) : IArchiveAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new ArchiveAgent( options );
  }

  /** Services specific to the Audience client */
  public getAudienceAgent(resourceParsing?: base.ResourceParsing) : IAudienceAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new AudienceAgent( options );
  }

  /** Preferences, user interface tab configuration */
  public getPreferenceAgent(resourceParsing?: base.ResourceParsing) : IPreferenceAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new PreferenceAgent( options );
  }

  /** License query, download, activation */
  public getLicenseAgent(resourceParsing?: base.ResourceParsing) : ILicenseAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new LicenseAgent( options );
  }

  /** User administration */
  public getUserAgent(resourceParsing?: base.ResourceParsing) : IUserAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new UserAgent( options );
  }

  /** Replication/Travel administration */
  public getReplicationAgent(resourceParsing?: base.ResourceParsing) : IReplicationAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new ReplicationAgent( options );
  }

  /** TimeZone maintenance */
  public getTimeZoneAgent(resourceParsing?: base.ResourceParsing) : ITimeZoneAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new TimeZoneAgent( options );
  }

  /** SMS and other external messaging systems */
  public getMessagingAgent(resourceParsing?: base.ResourceParsing) : IMessagingAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new MessagingAgent( options );
  }

  /** This agent can be used to import data into the system */
  public getImportAgent(resourceParsing?: base.ResourceParsing) : IImportAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new ImportAgent( options );
  }

  /** This agent can be used to manage the free text system */
  public getFreeTextAgent(resourceParsing?: base.ResourceParsing) : IFreeTextAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new FreeTextAgent( options );
  }

  /** This agent can be used to manage number allocation */
  public getNumberAllocationAgent(resourceParsing?: base.ResourceParsing) : INumberAllocationAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new NumberAllocationAgent( options );
  }

  /** Agent used for Customer Service methods */
  public getCustomerServiceAgent(resourceParsing?: base.ResourceParsing) : ICustomerServiceAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new CustomerServiceAgent( options );
  }

  /** Agent with Pocket specific functionality */
  public getPocketAgent(resourceParsing?: base.ResourceParsing) : IPocketAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new PocketAgent( options );
  }

  /** Agent lets you configure dashboard tiles and retrieve dashboard data */
  public getDashboardAgent(resourceParsing?: base.ResourceParsing) : IDashboardAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new DashboardAgent( options );
  }

  /** Agent that lets you configure dashboard tiles and retrieve dashboard data */
  public getDashAgent(resourceParsing?: base.ResourceParsing) : IDashAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new DashAgent( options );
  }

  /** Agent lets you configure targets, and retrieve targets */
  public getTargetsAgent(resourceParsing?: base.ResourceParsing) : ITargetsAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new TargetsAgent( options );
  }

  /** Agent used for retrieveing and setting favourites */
  public getFavouriteAgent(resourceParsing?: base.ResourceParsing) : IFavouriteAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new FavouriteAgent( options );
  }

  /** Database schema queries and changes */
  public getDatabaseAgent(resourceParsing?: base.ResourceParsing) : IDatabaseAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new DatabaseAgent( options );
  }

  /** Agent used for bulk update methods */
  public getBulkUpdateAgent(resourceParsing?: base.ResourceParsing) : IBulkUpdateAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new BulkUpdateAgent( options );
  }

  /** Agent used for Marketing functions, such as Forms */
  public getMarketingAgent(resourceParsing?: base.ResourceParsing) : IMarketingAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new MarketingAgent( options );
  }

  /** Manage and execute CRMScript functions. */
  public getCRMScriptAgent(resourceParsing?: base.ResourceParsing) : ICRMScriptAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new CRMScriptAgent( options );
  }

  /** Agent used for Ticket functions */
  public getTicketAgent(resourceParsing?: base.ResourceParsing) : ITicketAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new TicketAgent( options );
  }

  /** Agent used for CRUD operations on database tables. Read, add, update, delete rows in tables. */
  public getDatabaseTableAgent(resourceParsing?: base.ResourceParsing) : IDatabaseTableAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new DatabaseTableAgent( options );
  }

  /** Chat functions. Manage chat channels, sessions and messages. */
  public getChatAgent(resourceParsing?: base.ResourceParsing) : IChatAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new ChatAgent( options );
  }

  /** AI services, such as Translation, Statistics, Sentiment analysis, backed by calls to Public Cloud providers */
  public getAIAgent(resourceParsing?: base.ResourceParsing) : IAIAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new AIAgent( options );
  }

  /** Agent used to support migrating documents between different document-plugins, as single documents or batches. */
  public getDocumentMigrationAgent(resourceParsing?: base.ResourceParsing) : IDocumentMigrationAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new DocumentMigrationAgent( options );
  }

  /** Agent lets you configure workflow automation */
  public getWorkflowAgent(resourceParsing?: base.ResourceParsing) : IWorkflowAgent {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new WorkflowAgent( options );
  }



  /** The Contact Service. The service implements all services working with the Contact object */
  public getContactRestApi(resourceParsing?: base.ResourceParsing) : IContactRestApi {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new ContactRestApi( options );
  }


  /** The Person Service. The service implements all services working with the Person object. */
  public getPersonRestApi(resourceParsing?: base.ResourceParsing) : IPersonRestApi {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new PersonRestApi( options );
  }


  /** The Sale Entity contains the sale amount, currency, and sale members. Sales are linked to contacts, persons, and/or projects. */
  public getSaleRestApi(resourceParsing?: base.ResourceParsing) : ISaleRestApi {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new SaleRestApi( options );
  }


  /** The Project Service. The service implements all services working with the Project object */
  public getProjectRestApi(resourceParsing?: base.ResourceParsing) : IProjectRestApi {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new ProjectRestApi( options );
  }


  /** Appointments appear in the diary, and have links to a Contact/Person and possibly a Project or Sale. They have start and end time+date. */
  public getAppointmentRestApi(resourceParsing?: base.ResourceParsing) : IAppointmentRestApi {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new AppointmentRestApi( options );
  }


  /**  */
  public getDocumentRestApi(resourceParsing?: base.ResourceParsing) : IDocumentRestApi {
    const options = base.WebApiBase.FigureOutOptions(this._options);
    if( typeof(resourceParsing) == "number" )
        options.resourceParsing = resourceParsing;
    return new DocumentRestApi( options );
  }


}
