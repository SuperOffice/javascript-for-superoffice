...
describe("WebAPI tests", () => {
	...
	it("Can configure agents via webapi", done => {
		...
		const config: AxiosRequestConfig = { auth: { username: "ADM0", password: "" } };
		AgentsApi.configure(host, config);
		const agent = new WebhookAgent();
		agent.CreateDefaultWebhook().then(response => {
			assert.equal(response.WebhookId, 0);
			assert.equal(response.Name, null);
	...
	it("Can configure multiple agents using global defaults", done => {
		let scope = nock(host)
			.post("/api/v1/Agents/Webhook/CreateDefaultWebhook")
			.reply(200, defaultHook);

		AgentsApi.configure(host);
		let agent1 = new WebhookAgent();
	...
	it("Cannot create an agent with no global defaults", done => {
	...
	it("Can configure using an options object", done => {
		let scope = nock(host)
			.post("/api/v1/Agents/Webhook/CreateDefaultWebhook")
			.reply(200, defaultHook);

		let opts: WebApiOptions = { baseUrl: host };
		AgentsApi.configure(opts);
	...
	it("Can configure using a webAPi instance", done => {
		let scope = nock(host)
			.post("/api/v1/Agents/Webhook/CreateDefaultWebhook")
			.reply(200, defaultHook);

		let api = new AgentsApi(host);
		let agent = api.getWebhookAgent();
		agent.CreateDefaultWebhook().then(response => {
			assert.equal(response.WebhookId, 0);
			assert.equal(response.Name, null);
			assert.equal(response.State, WebhookState.Active);
	...
	it("Can configure hook using an axios config object", async () => {
	...
	it("can configure without parsing languages", done => {
		let scope = nock(host)
		...
		let opts: WebApiOptions = { baseUrl: host, cultureCode: "sv", languageCode: "sv" };
		let api = new AgentsApi(opts);
		api.authenticateWithPassword("ADM0", "");
		let agent = new WebhookAgent();
		agent.CreateDefaultWebhook({ languageCode: "" }).then(response => {
			assert.equal(response.WebhookId, 0);
			assert.equal(response.Name, null);
			assert.equal(response.State, WebhookState.Active);
			assert.equal(response.Type, "webhook");
	...
	it("can create a default hook without parsing languages and a password", done => {
		...
		let api = new AgentsApi(host);
		api.authenticateWithPassword("ADM0", "");
		let agent = new WebhookAgent(undefined, ResourceParsing.DoNotParse);
		agent.CreateDefaultWebhook().then(response => {
			assert.equal(response.WebhookId, 0);
			assert.equal(response.Name, null);
			assert.equal(response.State, WebhookState.Active);
